...
https://bitbonsai.github.io/claude-status/
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "input=$(cat); cwd=$(echo \"$input\" | jq -r '.workspace.current_dir'); cd \"$cwd\" 2>/dev/null || exit 0; output=\"$(whoami)\"; dir=\"${cwd/#$HOME/~}\"; output=\"${output} $(printf '\\033[34m%s\\033[0m' \"$dir\")\"; if git rev-parse --git-dir > /dev/null 2>&1; then branch=$(git branch --show-current 2>/dev/null || git rev-parse --short HEAD 2>/dev/null); [ -n \"$branch\" ] && output=\"${output} $(printf '\\033[90m%s\\033[0m' \"$branch\")\"; cc=$(git diff --name-only --diff-filter=U 2>/dev/null | wc -l | tr -d ' '); [ \"$cc\" -gt 0 ] 2>/dev/null && output=\"${output} $(printf '\\033[31m%s\\033[0m' \"${cc}⚠\")\"; mc=$(git diff --name-only 2>/dev/null | wc -l | tr -d ' '); [ \"$mc\" -gt 0 ] 2>/dev/null && output=\"${output} $(printf '\\033[33m%s\\033[0m' \"${mc}✎\")\"; uc=$(git ls-files --others --exclude-standard 2>/dev/null | wc -l | tr -d ' '); [ \"$uc\" -gt 0 ] 2>/dev/null && output=\"${output} $(printf '\\033[90m%s\\033[0m' \"${uc}?\")\"; sc=$(git diff --cached --name-only 2>/dev/null | wc -l | tr -d ' '); [ \"$sc\" -gt 0 ] 2>/dev/null && output=\"${output} $(printf '\\033[32m%s\\033[0m' \"${sc}✓\")\"; upstream=$(git rev-parse --abbrev-ref @{upstream} 2>/dev/null); if [ -n \"$upstream\" ]; then ahead=$(git rev-list --count @{upstream}..HEAD 2>/dev/null || echo 0); behind=$(git rev-list --count HEAD..@{upstream} 2>/dev/null || echo 0); [ \"$ahead\" -gt 0 ] && output=\"${output} $(printf '\\033[32m↑%s\\033[0m' \"$ahead\")\"; [ \"$behind\" -gt 0 ] && output=\"${output} $(printf '\\033[31m↓%s\\033[0m' \"$behind\")\"; fi; fi; [ -n \"$VIRTUAL_ENV\" ] && output=\"${output} $(printf '\\033[90m%s\\033[0m' \"$(basename \"$VIRTUAL_ENV\")\")\"; usage=$(echo \"$input\" | jq '.context_window.current_usage'); if [ \"$usage\" != \"null\" ]; then current=$(echo \"$usage\" | jq '.input_tokens + .cache_creation_input_tokens + .cache_read_input_tokens'); size=$(echo \"$input\" | jq '.context_window.context_window_size'); pct=$((current * 100 / size)); filled=$((pct / 10)); bar=\"\"; i=0; while [ $i -lt 10 ]; do if [ $i -lt $filled ]; then bar=\"${bar}█\"; else bar=\"${bar}░\"; fi; i=$((i + 1)); done; if [ $pct -ge 80 ]; then barcolor=\"\\033[31m\"; elif [ $pct -ge 60 ]; then barcolor=\"\\033[33m\"; else barcolor=\"\\033[32m\"; fi; output=\"${output} $(printf '\\033[90m∴\\033[0m') $(printf '\\033[38;2;76;97;90mcontext:\\033[0m') ${barcolor}${bar}$(printf '\\033[0m') ${barcolor}${pct}%%$(printf '\\033[0m')\"; fi; echo \"$output\""
  }
}
```
